...
if __name__ == '__main__':
    ...
    model = GCNEdgeBasedEdgeGenCluster(len(feature_columns), num_cluster=30, loss_weights=[5,0.05,0.5,0], device=device)
    optimizer = Adam(model.parameters(), lr=0.03, weight_decay=1e-5)
    ...
    for epoch in range(EPOCH):
        if (epoch) % 10 == 0:      
            with torch.no_grad():
                ...
                C = torch.tensor(C).to(device)
                model.add_connectivity(C)
                loss,loss_edge_class,loss_edge_regen,loss_cluster,loss_regularize = model(X)
                print(f'train loss: {loss}')
                writer.add_scalar('Loss/train', loss, epoch)
                writer.add_scalar('Loss_edge_class/train', loss_edge_class, epoch)
                writer.add_scalar('loss_edge_regen/train', loss_edge_regen, epoch)
                writer.add_scalar('loss_cluster/train', loss_cluster, epoch)
                writer.add_scalar('loss_regularize/train', loss_regularize, epoch)
                ...
                writer.add_scalar('Loss_edge_class/test', loss_edge_class, epoch)
                writer.add_scalar('loss_edge_regen/test', loss_edge_regen, epoch)
                writer.add_scalar('loss_cluster/test', loss_cluster, epoch)
                writer.add_scalar('loss_regularize/test', loss_regularize, epoch)

                # print(np.mean(SX.numpy()), np.std(SX.numpy()))
                torch.save(model, f'weights/m12i_model_edgegen_clustergen_32_32_epoch{epoch}.pth')
